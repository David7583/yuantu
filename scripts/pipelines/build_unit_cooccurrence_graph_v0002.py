# ============================================================
# File: build_unit_cooccurrence_graph_v0002.py
# 中文名: 对象级共现关系图构建脚本
# Version: v0002
#
# Layer: execution
...
# Purpose
#
# 基于 register（已登记结构单元）与结构共现事实（cooccurrence），
# 构建"对象级共现关系图"。当提供 register 时，节点来源仅限 register；
# 当使用免登记模式（--register 省略）时，节点从边数据中自动发现。
#
# What it does
#
# 1 读取 register JSONL，构建合法节点集合（unit_id → label）
# 2 读取 cooccurrence JSONL（单文件或目录），流式聚合边权重
...
#
# 1 不生成或修改 register
# 2 不对共现事实进行语义解释或重要性判断
# 3 不为下游兜底造节点（register 模式下不允许 cooccurrence 反向生成对象；免登记模式除外）
# 4 不回写任何上游产物，不修改原始数据资产
#
# v0002 changes
#
# 1 支持 --cooccurrence-dir 读取目录下所有 JSONL 文件（v0003 analyze 的 1:1 输出）
# 2 聚合使用 dict 累加，内存占用 = 唯一 pair 数 × ~200B，无硬门槛
# 3 新增 --output-jsonl 输出聚合后的 JSONL（流式，供 sync_to_graph 消费）
# 4 _extract_pair 适配 v0003 字段名（unit_id_a/unit_id_b）
# 5 _extract_count 适配 v0003 字段名（cooccurrence_count）
# 6 进度报告：每处理 100 个文件输出一次 stderr 进度
#
# Design decision
#
# 聚合使用内存 dict 是经过计算的选择：
# 5.5M 行 cooccurrence → 去重后约 100 万~200 万唯一 pair
# 每个 key (unit_id_a, unit_id_b) 约 100B + value int → 总计 ~200MB~400MB
...
# ALIAS_META
# ============================================================
# alias: build_unit_cooccurrence_graph_v0002
# family: build_unit_cooccurrence_graph
# role: object_graph_builder_cooccurrence
# version: v0002
# status: active
# entry_point: build_unit_cooccurrence_graph_v0002.py
# input:
#   - register_structural_units_jsonl
#   - cooccurrence_facts_jsonl (single file or directory of JSONL files)
#   - graph_policy_yaml_optional
# output:
#   - unit_cooccurrence_graph_json (graph object with nodes + edges)
#   - unit_cooccurrence_edges.jsonl (aggregated edges for sync_to_graph)
#   - run_meta_json
# depends_on:
#   - python_stdlib_only
# used_by:
...
GRAPH_FILENAME = "unit_cooccurrence_graph.json"
EDGES_JSONL_FILENAME = "unit_cooccurrence_edges.jsonl"
ARCHIVE_DIRNAME = "archive"
CURRENT_DIRNAME = "current"
RUN_META_DIRNAME = "_run_meta"

GRAPH_TYPE = "unit_cooccurrence"
SCRIPT_NAME = "build_unit_cooccurrence_graph_v0002"
...
def _build_nodes(register_path: Path) -> Tuple[Dict[str, Dict[str, Any]], int]:
    ...
    for row in _read_jsonl_stream(register_path):
        ...
        if row.get("__parse_failed__"):
            continue
        # Use unit_text as the matching key
...
# ============================================================
# Cooccurrence Parsing (v0003 compatible)
# ============================================================

def _extract_pair_from_cooccurrence_row(row: Dict[str, Any]) -> Optional[Tuple[str, str]]:
    """Extract (unit_text_a, unit_text_b) from a cooccurrence row.
    ...
    """
    candidates = [
        ("unit_text_a", "unit_text_b"),  # preferred: object-level text matching
        ("unit_id_a", "unit_id_b"),      # fallback: v0003 format (instance-level)
        ("unit_a", "unit_b"),
        ("a", "b"),
        ("u", "v"),
        ("left", "right"),
    ...
    if isinstance(pair, dict):
        v1 = pair.get("unit_text_a") or pair.get("a") or pair.get("unit_a") or pair.get("unit_id_a")
        v2 = pair.get("unit_text_b") or pair.get("b") or pair.get("unit_b") or pair.get("unit_id_b")
        if isinstance(v1, str) and isinstance(v2, str) and v1.strip() and v2.strip():
            return v1.strip(), v2.strip()
...
def _extract_count_from_cooccurrence_row(row: Dict[str, Any]) -> int:
    """Extract count from a cooccurrence row.

    v0003 field: cooccurrence_count
    Legacy fields: count, cooccurrence, freq, etc.
    """
    for k in ("cooccurrence_count", "count", "cooccurrence", "freq", "frequency", "n", "weight"):
        v = row.get(k)
        if isinstance(v, int):
            return v
...
def _aggregate_edges(
    ...
    """Stream through all cooccurrence files, aggregate by canonical (u, v) pair.

    When node_ids is None (no register), all edges are accepted and labels
    are extracted from unit_text_a/unit_text_b fields in the edge data.

    Returns:
        edge_counts: dict mapping (u, v) → total count
    ...
    for file_idx, fpath in enumerate(input_files):
        for row in _read_jsonl_stream(fpath):
            ...
            pair = _extract_pair_from_cooccurrence_row(row)
            if not pair:
                stats["rows_dropped_parse_failed"] += 1
                continue
            ...
            cnt = _extract_count_from_cooccurrence_row(row)

            # Directed: preserve original text position order from analyze v0003
            # unit_text_a = earlier in text, unit_text_b = later in text
            key = (a, b)
        ...
        if verbose and (file_idx + 1) % PROGRESS_INTERVAL == 0:
            print(
                f"[COOCCURRENCE-BUILD] Progress: {file_idx + 1}/{len(input_files)} files, "
    ...
    if verbose:
        print(
            f"[COOCCURRENCE-BUILD] Aggregation done: {stats['files_processed']} files, "
            f"{stats['rows_read']} rows read, "
...
def _prune_edges(
    ...
    for (u, v), cnt in edge_counts.items():
        if u == v:
            prune_details["self_loops_removed"] += 1
            continue
        cleaned[(u, v)] = cnt

    # Min weight filter
    filtered: Dict[Tuple[str, str], int] = {}
    for key, cnt in cleaned.items():
        if cnt >= min_w:
            filtered[key] = cnt
        else:
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Build object-level cooccurrence graph from register + cooccurrence facts (v0002)."
    )
    ...
    input_group.add_argument(
        "--cooccurrence",
        default=None,
        help="Path to a single cooccurrence facts JSONL file.",
    )
    input_group.add_argument(
        "--cooccurrence-dir",
        default=None,
        help="Path to directory containing cooccurrence JSONL files (v0003 per-file output).",
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.cooccurrence:
        ...
        if not cooccurrence_path.exists():
            print(f"[ERROR] cooccurrence not found: {cooccurrence_path}", file=sys.stderr)
            return 1
        input_files = [cooccurrence_path]
        input_mode = "single_file"
        input_source = str(cooccurrence_path)
    elif args.cooccurrence_dir:
        cooccurrence_dir = Path(args.cooccurrence_dir)
        if not cooccurrence_dir.exists():
            print(f"[ERROR] cooccurrence directory not found: {cooccurrence_dir}", file=sys.stderr)
            return 1
        input_files = _glob_jsonl_files(cooccurrence_dir)
        if not input_files:
            print(f"[ERROR] No .jsonl files found in: {cooccurrence_dir}", file=sys.stderr)
            return 1
        input_mode = "directory"
        input_source = str(cooccurrence_dir)
    else:
        print("[ERROR] No cooccurrence input specified.", file=sys.stderr)
        return 1
    ...
    if register_path is not None:
        ...
        nodes_map = {}
        register_rows = 0
        node_ids = None  # signals: accept all edges
    ...
    if verbose:
        print(f"[COOCCURRENCE-BUILD] Input mode: {input_mode}, files: {len(input_files)}", file=sys.stderr)
        print(f"[COOCCURRENCE-BUILD] Register mode: {register_mode}, nodes: {len(nodes_map)}", file=sys.stderr)

    # 2) Stream-aggregate edges
    edge_counts, discovered_labels, agg_stats = _aggregate_edges(input_files, node_ids, verbose)

    # Merge discovered labels into nodes_map (fills gaps or builds from scratch)
    ...
    # 4) Statistics
    # Only count nodes that actually appear in edges
    ...
    # Nodes: only include nodes that participate in edges (active nodes)
    ...
    run_meta_obj: Dict[str, Any] = {
        ...
        "inputs": {
            ...
            "cooccurrence_source": input_source,
        ...
        "aggregation_stats": {
            ...
            "cooccurrence_rows_read": agg_stats["rows_read"],
            "cooccurrence_rows_used": agg_stats["rows_used"],
            "cooccurrence_rows_dropped_not_in_register": agg_stats["rows_dropped_not_in_register"],
            "cooccurrence_rows_dropped_parse_failed": agg_stats["rows_dropped_parse_failed"],
            "cooccurrence_files_processed": agg_stats["files_processed"],
